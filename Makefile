...
DOCKER_BUILD_PATH	:= /home/h8ck3r/go/src/github.com/h8ck3r/gscan
PROJECT_NAME		:= gscan
...
DOCKERGOCOMMAND := @docker run $(GPU) -v $(CURDIR):$(DOCKER_BUILD_PATH)/$(PROJECT_NAME) --userns host -w $(DOCKER_BUILD_PATH)/$(PROJECT_NAME) -e GOOS=$(DOCKER_GOOS) -e GOARCH=$(DOCKER_GOARCH) -e GO111MODULE=$(DOCKER_GO111MODULE) -e CGO_ENABLED=$(DOCKER_CGO_ENABLED) --rm -it $(DOCKER_IMAGE_NAME) go
